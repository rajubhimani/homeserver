...
- `ollama` — plain `up ollama` silently no-op'd (`no service selected`) because it was gated behind the `docker-ollama` compose profile (kept deliberately at the time for a future host-native-GPU option). **Since removed** — `ollama` is now an unprofiled `daily`-tier service like any other; no `--profile` flag needed. See `docs/services/open-webui.md`.
