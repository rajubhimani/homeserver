...
- **SERVICES_EXTRA** (`up all` or individually): dozzle → dockge → openproject → paperless → mealie → audiobookshelf → invoiceshelf → appflowy → outline → bookstack → mattermost → rocketchat → zulip → ntfy → orangehrm → nocodb → documenso → penpot → supabase → crowdsec
