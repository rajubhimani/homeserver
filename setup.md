...
[Browser Hub](docs/services/browser-hub.md "One login, then pick from ten real browsers (Firefox, Chromium, Brave, Mullvad Browser, LibreWolf, Zen, Helium, Chrome, Edge, Vivaldi) running on the server and controlled from any device — the page loads and the traffic goes out from your server, so it reaches sites your local connection can't. Each browser only exists at its own /path/ here, not its own web address.") ·
[Firefox](docs/services/firefox.md "General-purpose remote Firefox — part of the Browser Hub, one login for all ten browsers here.") ·
[Chromium](docs/services/chromium.md "Remote Chromium — for sites/tools that specifically need Chrome/Chromium. Part of the Browser Hub, one login for all ten browsers here.") ·
[Brave](docs/services/brave.md "Privacy-focused, ad-blocking-by-default remote browser. Part of the Browser Hub, one login for all ten browsers here.") ·
[Mullvad Browser](docs/services/mullvad-browser.md "Hardened, anti-fingerprinting Firefox fork built with the Tor Project — does not itself route traffic through the Tor network. Part of the Browser Hub, one login for all ten browsers here.") ·
[LibreWolf](docs/services/librewolf.md "Privacy-hardened Firefox fork with telemetry removed and uBlock Origin built in. Part of the Browser Hub, one login for all ten browsers here.") ·
[Zen](docs/services/zen.md "Firefox-based browser with vertical tabs, split view and a clean, modern layout. Part of the Browser Hub, one login for all ten browsers here.") ·
[Helium](docs/services/helium.md "Privacy-first Chromium-based browser with ad-blocking built in and Google services stripped out. Part of the Browser Hub, one login for all ten browsers here.") ·
[Chrome](docs/services/chrome.md "The official Google Chrome, for sites that only work properly in Chrome. Sends usage data to Google — use it for compatibility, not everyday privacy. Part of the Browser Hub, one login for all ten browsers here.") ·
[Edge](docs/services/edge.md "The official Microsoft Edge, for Microsoft 365 and sites that insist on Edge. Sends usage data to Microsoft — use it for compatibility, not everyday privacy. Part of the Browser Hub, one login for all ten browsers here.") ·
[Vivaldi](docs/services/vivaldi.md "Power-user Chromium browser with tab stacking, built-in mail and calendar, and heavy customisation. Part of the Browser Hub, one login for all ten browsers here.") ·
...
├── librewolf/        (config/) — same hub, browser.<domain>/librewolf/
├── zen/              (config/) — same hub, browser.<domain>/zen/
├── helium/           (config/) — same hub, browser.<domain>/helium/
├── chrome/           (config/) — same hub, browser.<domain>/chrome/
├── edge/             (config/) — same hub, browser.<domain>/edge/
├── vivaldi/          (config/) — same hub, browser.<domain>/vivaldi/
├── portainer/        (data/)
