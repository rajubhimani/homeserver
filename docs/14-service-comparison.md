...
| **AppFlowy** | ○ down | extra | 8 | **~382MB** total (documented; appflowy-cloud alone ~121MB) — only 3/8 containers (db/minio/redis, ~106MB) are up this session, the app itself is still down | appflowy_cloud 0.16.5 — unconfirmed (search only surfaced the separate desktop-client versioning) |
