...
`DISABLE_REGISTRATION` in `.env`, default `true` (closed) — `.env`'s `RUNNER_UUID`/`RUNNER_TOKEN` are unrelated (that's the CI runner's own registration, not user signup). Create every account with the same CLI command as above (drop `--admin` for a regular user), or from the web UI: **Site Administration → Users → Create User Account**. Set to `false` only if open public self-signup is actually wanted.
...
1. In Forgejo, go to **Site Administration → Actions → Runners → Create new Runner** and copy both the **UUID** and **Token** shown (only shown once).
2. Set `RUNNER_UUID`/`RUNNER_TOKEN` (and optionally `RUNNER_LABELS`) in `services/forgejo/.env`.
3. `uv run homeserver.py dev up forgejo`

**This pin (`code.forgejo.org/forgejo/runner:13.0.0`) uses the newer `server.connections` config method, not the older `register --token <pool-token>` flow** — confirmed live, the runner logs `` `register` has been deprecated; declare connections in the runner configuration instead `` on every start, and Forgejo's own admin UI now generates a per-runner UUID+token pair for exactly this schema rather than a reusable pool token. Pointing the old `register --token` command at one of these new-style tokens fails immediately with `invalid_argument: runner registration token not found` — not because the token is wrong, but because it was never the right *kind* of credential for that command (confirmed by checking Forgejo's own `action_runner_token` table directly: the UI-generated token never lands there at all, it's stored per-runner instead). `compose.yml`'s command script writes a `config.yaml` with `runner.labels`, `container.docker_host`, and `server.connections.forgejo.{url,uuid,token}`, then runs `forgejo-runner daemon --config /data/config.yaml` — no separate register step, and (unlike the old flow) no `.runner` file persisted or checked on later boots: the config (token included) is rewritten from `.env` and the daemon restarted fresh on every container start, since this connection method has no first-boot-only state to skip past.

The image's own default command (`forgejo-runner` with no subcommand) just prints help text and exits 0, which `restart: unless-stopped` loops forever without ever connecting — this is why `compose.yml` overrides `command:` with the config-then-daemon script above instead of relying on the image default.
