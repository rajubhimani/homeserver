...
Composition via Child Workflows. `BatchProcessingWorkflow` starts 3 `GreetSourceWorkflow` children concurrently (`asyncio.gather` + `execute_child_workflow`), each with its own Workflow ID and Event History — one child's failure doesn't corrupt the parent's or another child's state. Compare against Airflow's [`example_parallel_tasks`](../airflow/example_parallel_tasks.md): similar fan-out shape at a glance, but each child here is independently durable and independently queryable, not just a step inside one shared DAG run.
