...
**Real-world problem:** you're writing a real Activity call and need to know what `execute_activity()`/`RetryPolicy` actually support — a timeout, a retry cap, cancellation behavior — without digging through SDK source or guessing from a partial example that only ever showed 2 of the 15 available options.
