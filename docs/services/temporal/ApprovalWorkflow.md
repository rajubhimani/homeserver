...
Durable state across arbitrarily long waits, resumed by an external Signal, inspectable at any time via a Query (Signals push data *in*; Queries read state back *out* without affecting execution — the two are normally taught as a pair). The direct parallel to Airflow's built-in [`example_human_in_the_loop`](../airflow/example_human_in_the_loop.md) — same underlying idea, different mechanism: this is hand-written workflow code waiting on a Signal, Airflow's HITL is a single operator that does the same job with no Signal-handling code.
