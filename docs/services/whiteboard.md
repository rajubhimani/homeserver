...
**Pinned version:** `ghcr.io/nextcloud-releases/whiteboard:stable`. Upstream doesn't publish clean numbered release tags (GHCR listing at the time this was added was a mix of beta/daily/commit-hash tags, no plain `v1.x.x`) — `:stable` is upstream's own documented production tag. Resolved to **1.5.9** at the time this was added, confirmed live via the container's own startup log (`whiteboard@1.5.9`) — matches the Nextcloud `whiteboard` app's version exactly, since the app and this backend are versioned/released together.
