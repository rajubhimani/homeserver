...
- All persistent data lives in named Docker volumes (`plausible-postgres`, `plausible-clickhouse-data`, `plausible-clickhouse-logs`, `plausible-data`) rather than `service_data/data/` bind mounts — matches this stack's DB-data convention (see the `homeserver-postgres` skill) and upstream's own default.
