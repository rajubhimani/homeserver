...
- **`MAXMIND_LICENSE_KEY` (`.env`) is unset by default — Locations stats silently don't work without it.** Confirmed live: with it blank, the app logs `[locus] [geolocation] database failed to load (:remote): {:http, 401, "Unauthorized"}` on a recurring retry (roughly hourly), forever, since MaxMind's GeoLite2 databases have required a free registered account + license key since December 2019 — there's no way to fetch one anonymously anymore. Nothing else breaks; page view/visitor tracking works fine without it, only the **Locations** panel has no country/city data to show. To fix: sign up at [maxmind.com](https://www.maxmind.com/en/geolite2/signup) (free), generate a license key under **My License Keys**, set `MAXMIND_LICENSE_KEY` in `.env`, restart.
