...
The hub's own card (`services.json` slug `browser`) is marked `"virtual": true` — it has a landing-page presence but no `services/browser/compose.yml`; `homeserver.py` excludes `virtual` entries from `SERVICES_MIN`/`CORE`/`DAILY`/`EXTRA`/`MANUAL` and from automatic category/subcategory `SERVICE_GROUPS` derivation, so `up all`/`up group:browsers`/etc. never try to docker-compose a directory that doesn't exist.
