...
It's a [PEP 723](https://peps.python.org/pep-0723/) inline-metadata script — `uv run` installs its one dependency (`uptime-kuma-api`) into an ephemeral env automatically, nothing to add to the repo's own Python deps. It prompts for your Uptime Kuma username/password interactively (not read from any env file, not persisted anywhere) — plus a 2FA code if the account has two-factor login enabled (before 2026-09-26 the script didn't handle 2FA: the server answers a correct password with `tokenRequired` instead of an error, the socket stays unauthenticated, and the script died 60s later with `Timed out while waiting for event Event.DOCKER_HOST_LIST`) and is safe to re-run: existing docker host/notification/monitors are matched by name and skipped, so re-running after starting new services only adds monitors for the new ones.

**`--all` — cover every service the repo knows about, not just what's currently running.** The default mode only sees containers `docker ps` reports right now, so an opt-in-tier service you haven't started yet (or a MIN/CORE one that happens to be stopped) never gets a monitor at all. `--all` instead parses every `container_name:` out of every `services/*/compose.yml` (skipping `services/nginx/` — that's `nginx-proxy-manager`, a mutually-exclusive alternative to `nginx-plain`, not part of any tier — and every one-shot init/permission container — detected automatically as any service with `restart: "no"` or `restart: on-failure` in its compose file, e.g. `firefly-permissions`/`erpnext-configurator`, which are *supposed* to exit and would otherwise sit permanently "down". This was a hand-maintained name list until 2026-09-26, when it was found to have drifted: `atuin-permissions`, `authentik-permissions` and 8 others had monitors showing down; `--prune` now deletes monitors for one-shots too), looks up each container's tier via `services.json`, and creates a monitor for literally all of them: **enabled** for `min`/`core` (the always-on tiers — 52 containers as of this pass, including ones like `forgejo`/`wg-easy`/`plausible` that were stopped at the time), **created but paused** for everything else (117 containers across `daily`/`office`/`automation-ai`/`extra`/`manual`) so an intentionally-stopped opt-in service doesn't spam down alerts. Flip a monitor's own **Active** toggle in the UI later if you start using one of those regularly — the script won't touch it again once it exists (same by-name skip as the default mode).
...
```bash
uv run services/uptime-kuma/setup-monitors.py --all --prune
```
