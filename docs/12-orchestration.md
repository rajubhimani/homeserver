...
| Capping concurrent executions | `example_max_active_runs` (`max_active_runs=1`) | `ConcurrencyLimitedWorkflow` (`Worker(max_concurrent_activities=20)`, process-wide, not per-workflow — a real permanent cap now, not just documented) | `pool=` on `@asset`/`@op` (documented in `reference_asset`/`reference_op`) — **not demoed live here**: it needs a `dagster.yaml` `concurrency:` block, which is baked into the shared `dagster-webserver`/`dagster-daemon` image at build time, so demoing it means editing that file and rebuilding both containers, not just a `definitions.py` restart like every other example in this repo |
...
| Fan-out / parallelism | `example_parallel_tasks` | `BatchProcessingWorkflow` (`asyncio.gather` children) | `fan_out` — 3 independent assets, verified running concurrently via event-log timestamps |
| Backfill / historical reprocessing | `example_backfill` (whole-DAG-run) | — (not a Temporal concept — see "Backdated / historical processing" above) | `daily_sales` (per-partition) / `daily_sales_single_run` (`BackfillPolicy.single_run()`, whole range in one run) |
...
| Runtime-determined number of parallel units | `example_dynamic_task_mapping` (`.expand()`, mapped instances live only for that one run) | — | `process_uploaded_file` (`DynamicPartitionsDefinition`, durable/named partitions, not tied to one run) |
...
| Custom reusable notification callback | `example_custom_notifier` (`BaseNotifier` subclass) | — (a workflow calls an Activity directly in its own `except` block instead — no separate callback-registration mechanism needed) | `log_job_success` (`@success_hook`, fires per-op not per-job) |
| Running short/cheap work without a full round-trip | `PythonVirtualenvOperator` isolates dependencies, not overhead — still a full task | `LocalActivityWorkflow` (`execute_local_activity()`, same worker process, no Task Queue round-trip, far fewer Event History entries) | — |
| Mid-flight cancellation reaching running work | — (no built-in equivalent; a task either completes or is killed) | `CancelableWorkflow` (`WAIT_CANCELLATION_COMPLETED`, delivered into a heartbeating Activity, not just abandoned) | — |
| Handing work to a separate external process, resumed by a token | — | `AsyncCompletionWorkflow` (`activity.raise_complete_async()` + a task token, completed later by anyone holding it) | — |
| Isolated per-task dependency environment | `PythonVirtualenvOperator` (a fresh venv per run, rebuilt every time unless cached) | — (isolation means a separate Worker deployment, not a per-call sandbox) | `docker_executor` (a whole container per step, heavier isolation, same underlying motivation) |
| Storage-agnostic path abstraction | `example_object_storage_path` (`ObjectStoragePath`, same code targets `file://`/`s3://`/`gs://`) | — (bring your own client library inside an Activity) | I/O managers (`FilesystemIOManager` in `report_pipeline`) — pluggable storage for asset *outputs* specifically, not a general-purpose path object |
| Declarative flow control (skip/gate without a branch router) | `example_flow_control` (`ShortCircuitOperator`, `LatestOnlyOperator`, `BranchDayOfWeekOperator`) | — | — |
